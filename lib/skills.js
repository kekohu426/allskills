...
const DDD_DIR = path.join(ROOT, "document-driven-development-main", "skills");
...
function getSourceInfo(skillPath) {
  ...
  if (skillPath.includes(`${path.sep}document-driven-development-main${path.sep}`)) {
    const repoPath = path.relative(
      path.join(ROOT, "document-driven-development-main"),
      path.dirname(skillPath)
    );
    return {
      sourceLabel: "document-driven-development",
      sourceUrl: `https://github.com/s87343472/document-driven-development/tree/main/${repoPath.replace(/\\/g, "/")}`,
      license: "MIT"
    };
  }
  return {

...
export default function SkillGeneratorDe() {
  ...
  const handleGenerate = async () => {
    ...
    if (!apiKey && usage.count >= DAILY_LIMIT) {
      setShowApiKey(true);
      setError(
        `Tageskontingent aufgebraucht (${DAILY_LIMIT}x). Gib deinen GLM API Key ein, um weiterzumachen.`
      );
      return;
    }
      ...
      <section className="section">
        <div className="generator-card">
          <div className="generator-card__header">
            <h2>Beschreibe deinen Skill</h2>
            <p>Erkläre Aufgabe, Domain und Ablauf, damit die Ausgabe produktionsreif ist.</p>
          </div>

          <textarea
            className="generator-input"
            placeholder="Beispiel: Ein Skill, der API-Dokumentation schreibt - mit Endpunkten, Request/Response, Fehlercodes und Auth..."
            value={input}
          ...
          {showApiKey && (
            <div className="api-key-section">
              <label>GLM API Key (optional, unbegrenzte Nutzung)</label>
              <input
                type="password"
          ...
          <div className="generator-actions">
            <span className="generator-hint">Heute verfügbar: {remaining} / {DAILY_LIMIT}</span>
            <button className="btn btn--primary" onClick={handleGenerate} disabled={loading || !input.trim()}>
              {loading ? "Wird erstellt..." : "Skill generieren"}
            </button>
          </div>
        </div>
            ...
            <div className="generator-result__header">
              ...
              {score !== null && (
                <div className="generator-score">
                  Qualitäts-Score: <strong>{score}</strong>/100
                </div>
              )}
            </div>
            <pre className="generator-output">{result}</pre>
            <div className="generator-result__actions">
              <button className="btn" onClick={handleCopy}>Kopieren</button>
              <button className="btn btn--ghost" onClick={handleDownload}>SKILL.md herunterladen</button>
            </div>
          </div>
        )}
        ...
        <div className="generator-how">
          ...
          <div className="generator-steps">
            <div className="generator-step">
              ...
              <h4>Beschreiben</h4>
              <p>Definiere Ziele und Auslöser des Skills.</p>
            </div>
            <div className="generator-step">
